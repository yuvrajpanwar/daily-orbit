...
@section('content')
    <div class="container-fluid">
        <div class="row justify-content-center">
            <div class="col-12">
                <h3 class="page-title"> Advisories (<span id="totalAdvisoriesCount"></span>) </h3>
            </div>
        </div>
    </div>
    ...
    <div class="container-fluid mb-4">
        <div class="row">
            <div class="col-12 d-flex justify-content-end">
                <a href="{{ route('add-advisory') }}"><button class="btn btn-primary">Add New Advisory</button></a>
            </div>
        </div>
    </div>
    ...
    <div class="container-fluid mb-4">
        ...
        <table id="advisoriesTable" class="table table-striped" style="width:100%">
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <p><strong>Advisory Title :</strong></p>
                    <p id="modalAdvisoryTitle"></p>
                    <p><strong>Advisory Description:</strong></p>
                    <p id="modalAdvisoryDescription"></p>
                    <p><strong>Author : </strong> <span id="modalAdvisoryAuthor"> </span></p>
                    <p><strong>Company : </strong> <span id="modalAdvisoryCompany"></span></p>
                </div>
                <div class="modal-footer">
        ...
        $(document).ready(function() {
            ...
            function createTable() {
                $('#advisoriesTable').DataTable({
                    ...
                    "ordering": false,
                    "ajax": {
                        "url": "{{ route('fetch-all-advisories') }}",
                        "data": function(d) {
                            d._token = "{{ csrf_token() }}";
                        },
                        "dataSrc": function(json) {
                            // Update the total advisories count
                            $('#totalAdvisoriesCount').text(json.recordsTotal);
                            return json.data;
                        }
                    },
                    "columns": [{
                        ...
                        $(row).on('click', function(event) {
                            ...
                            if (!$(event.target).hasClass('edit-btn') && !$(event.target)
                                .hasClass('delete-btn') && !$(event.target).hasClass(
                                    'visibility-checkbox')) {
                                $('#modalAdvisoryTitle').text(data.advisory_title);
                                $('#modalAdvisoryDescription').text(data.advisory_description);
                                $('#modalAdvisoryAuthor').text(data.author);
                                $('#modalAdvisoryCompany').text(data.company);
            ...
            $(document).on('click', '.edit-btn', function(event) {
                ...
                window.location.href = `edit-advisory/${id}`;
            });
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            if (response.status === 'success') {
                                $('#advisoriesTable').DataTable().ajax.reload();
                                // alert("Advisory deleted successfully.");
                            } else {
                                alert("An error occurred while deleting the advisory.");
                            }
                        }
                    });
                }
            ...
            $(document).on('change', '.visibility-checkbox', function() {
                ...
                $.ajax({
                    url: `update-advisory-visibility/${id}`,
                    type: 'POST',
                    data: {
                        _token: "{{ csrf_token() }}",
                    ...
                    success: function(response) {
                        if (response.status === 'success') {
                            // alert("advisory visibility updated successfully.");
                        } else {
                            alert("An error occurred while updating the advisory visibility.");
                        }
                    },
